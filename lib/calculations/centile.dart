...
double sdsToCentile(double sds){
  ...
  double pdf = (1.0 / sqrt(2 * pi)) * exp(-0.5 * z * z); // PDF: Probability Density Function
  double cdfApprox = 1.0 - pdf * (b1 * t + b2 * pow(t, 2) + b3 * pow(t, 3) + b4 * pow(t, 4) + b5 * pow(t, 5));

  double probability;
  if (z >= 0) {
    probability = cdfApprox;
  } else {
    probability = 1.0 - cdfApprox; // Due to symmetry: P(X <= z) = 1 - P(X <= -z) for z < 0
  }
