...
double calculateDeltaSodium({
  required double
      infusateSodiumConcentration, // Infusate sodium concentration in mmol/L
  ...
  if ((totalBodyWater + 1) == 0) {
    throw ArgumentError(
        "totalBodyWater + 1 cannot be zero, as it would lead to division by zero.");
  }

  // Formula: (infusateSodiumConcentration - plasmaSodium) / (totalBodyWater + 1)
  double deltaSodium =
      (infusateSodiumConcentration - plasmaSodium) / (totalBodyWater + 1);
