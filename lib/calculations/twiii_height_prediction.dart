...
(double, double) predictAdultHeight({
  ...
  if (sex == Sex.male) {
    ...
    if (menarchealStatus) {
      ...
      relevantData
          .where((item) => item.menarchealStatus == 'postmenarcheal')
          .toList();
    } else {
      // Filter for premenarcheal data
      relevantData
          .where((item) => item.menarchealStatus == 'premenarcheal')
          .toList();
    }
  } else {
  ...
  for (final dataEntry in relevantData) {
    ...
    if (chronologicalAge == entryAgeLowerBound) {
      double predictedAdultHeight = height * dataEntry.heightCoefficient +
          ...
          dataEntry.constant;
      double predictedAdultHeightAdjustedForMidParentalHeight =
          predictedAdultHeight;
      if (useMidParentalHeight) {
        predictedAdultHeightAdjustedForMidParentalHeight =
            adjustPredictedFinalHeightForMidParentalHeight(
                predictedAdultHeight, midParentalHeight!);
      }
      return (
        predictedAdultHeight,
        predictedAdultHeightAdjustedForMidParentalHeight
  ...
  if (chronologicalAge >= lastEntryLowerBound) {
  ...
  TWIIIAdultHeightPrediction upper =
      relevantData.last; // Should be updated in loop

  for (int i = 0; i < relevantData.length - 1; i++) {
    // Current entry's lower bound
    final double currentEntryLowerBound =
        _parseAgeToDouble(relevantData[i].age);
    // Next entry's lower bound
    final double nextEntryLowerBound =
        _parseAgeToDouble(relevantData[i + 1].age);

    if (chronologicalAge >= currentEntryLowerBound &&
        chronologicalAge < nextEntryLowerBound) {
  ...
  double interpolate(double lowerValue, double upperValue) {
    ...
    final t = (chronologicalAge - lowerBoundAgeForLowerEntry) /
        (lowerBoundAgeForUpperEntry - lowerBoundAgeForLowerEntry);
  ...
  final interpolatedChronologicalAgeCoefficient = interpolate(
      lower.chronologicalAgeCoefficient, upper.chronologicalAgeCoefficient);
...
double adjustPredictedFinalHeightForMidParentalHeight(
    double predictedAdultHeight, double midParentalHeight) {
  double predictedAdultHeightAdjustedForMidParentalHeight =
      predictedAdultHeight;
  if (midParentalHeight < 168) {
    predictedAdultHeightAdjustedForMidParentalHeight -=
        (168 - midParentalHeight) / 3;
  } else {
    predictedAdultHeightAdjustedForMidParentalHeight +=
        (midParentalHeight - 168) / 3;
  }
