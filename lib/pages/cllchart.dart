  ...
  Widget build(BuildContext context) {
    return Column(children: [
      SfCartesianChart(
        ...
        primaryYAxis:
            NumericAxis(title: AxisTitle(text: 'Size (cm)'), minimum: 2.0),
        series: <CartesianSeries>[
          LineSeries<FetalCLLData, int>(
            dataSource: FetalCLLData.dataList,
            xValueMapper: (FetalCLLData data, _) => data.gestationalWeeks,
            yValueMapper: (FetalCLLData data, _) =>
                widget.isWidth ? data.meanWidth : data.meanLength,
            color: Colors.blue,
          ...
          LineSeries<FetalCLLData, int>(
            ...
            xValueMapper: (FetalCLLData data, _) => data.gestationalWeeks,
            yValueMapper: (FetalCLLData data, _) =>
                widget.isWidth ? data.meanWidthOneSDS : data.meanLengthOneSDS,
            color: Colors.blue,
            name: '+1 SDS',
            dashArray: [5, 5],
          ),
          LineSeries<FetalCLLData, int>(
            ...
            xValueMapper: (FetalCLLData data, _) => data.gestationalWeeks,
            yValueMapper: (FetalCLLData data, _) =>
                widget.isWidth ? data.meanWidthTwoSDS : data.meanLengthTwoSDS,
            color: Colors.blue,
            name: '+2 SDS',
            dashArray: [5, 5],
          ),
          LineSeries<FetalCLLData, int>(
            dataSource: FetalCLLData.dataList,
            xValueMapper: (FetalCLLData data, _) => data.gestationalWeeks,
            yValueMapper: (FetalCLLData data, _) => widget.isWidth
                ? data.meanWidthThreeSDS
                : data.meanLengthThreeSDS,
            color: Colors.blue,
            name: '+3 SDS',
            dashArray: [5, 5],
          ),
          if (widget.showScatterPoint)
            ScatterSeries<ScatterData, int>(
              dataSource: [
                ScatterData(widget.selectedGestationWeek, widget.cll)
              ], // Use user input
        ...
        title: ChartTitle(
            text: widget.isWidth
                ? 'Clitoral Width for Gestation'
                : 'Clitoral Length for Gestation'),
      ),
      const Padding(
